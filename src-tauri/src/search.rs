...
fn extract_snippet(body: &str, query_lower: &str, terms: &[&str]) -> String {
    // `body.to_lowercase()` can change byte counts (German ß → ss, Turkish
    // İ → i̇, plus a handful of other locale-aware folds), so byte offsets
    // returned by `body_lower.find(needle)` don't reliably map back into
    // `body`. We instead search the original text directly with a
    // case-insensitive window comparison, returning the byte index in
    // `body` so window slicing stays char-aligned.
    let find_ci = |needle: &str| -> Option<usize> {
        if needle.is_empty() { return Some(0); }
        for (start_byte, _) in body.char_indices() {
            let mut hay = body[start_byte..].chars().flat_map(|c| c.to_lowercase());
            let mut nee = needle.chars();
            let matched = loop {
                match (hay.next(), nee.next()) {
                    (_, None) => break true,
                    (Some(a), Some(b)) if a == b => continue,
                    _ => break false,
                }
            };
            if matched { return Some(start_byte); }
        }
        None
    };

    let start = match find_ci(query_lower) {
        Some(i) => i,
        None => terms
            .iter()
            .filter_map(|t| find_ci(t))
            .min()
            .unwrap_or(0),
    };

    // Window in characters → byte boundaries. ~60 chars before, ~60 chars
    // (plus the matched query) after, all aligned to UTF-8 char boundaries
    // so slicing into `body` never panics mid-codepoint.
    let half = 60usize;

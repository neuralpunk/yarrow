...
- **anime.js v4 + motion vocabulary.** New `src/lib/anim.svelte.ts` centralises every animation (`popIn`, `pulseRing`, `staggerFadeUp`, `toastSlideIn`, `drawPath`, `sweepClip`, `widthSlide`, `radialSpawn`). All paths honour `prefers-reduced-motion`. (`package.json`, `anim.svelte.ts`)
- **Motion pass across the app.** Modal entrance, command palette open, toast slide-in, note-list filter cascade, fork moment timeline, focus-mode rail collapse, theme-swap ink wash, auto-checkpoint pulse, connection-graph radial spawn. (`Modal.svelte`, `CommandPalette.svelte`, `AppShell.svelte`, `NoteList.svelte`, `ForkMoment.svelte`, `theme.svelte.ts`, `NoteEditor.svelte`, `ConnectionGraph.svelte`)
- **Inline Wikilink Picker.** Typing `[[` opens a two-row picker (serif title, mono slug · last-edited · tag count) with a "Create *query*" fallback that scaffolds the missing note via `yarrow:wikilink-create`. Coexists with right-click → Insert. (`wikilinkAutocomplete.ts`, `NoteEditor.svelte`, `AppShell.svelte`, `index.css`)
- **Selection Toolbar.** Floating bar over text selection — Bold · Italic · Code · `[[ ]]` · Highlight · `??`. Spring entrance, Esc dismisses, edits routed through `yarrow:editor-replace-range`. (`selectionToolbar.ts`, `SelectionToolbar.svelte`, `NoteEditor.svelte`, `AppShell.svelte`)
- **Inline Glossary — file-backed, edited in Settings.** A new **Settings → Glossary** pane is the sole authoring surface: add term + definition pairs, edit in place, remove with a click. Entries are persisted to a single self-contained `.yarrow/glossary.json` (tracked in git, so the glossary travels with the workspace). The editor underlines every occurrence of a defined term across all notes — case-insensitive, word-boundary-aware, longest-term-first so "data structure" wins over "data" — and shows the definition in a hover tooltip. Each save dispatches `yarrow:glossary-updated`; open editors hot-swap their decoration compartment without remounting. Replaces the prior frontmatter-flag mechanism — no source-note linkage, no `glossary: true` frontmatter field, no right-click toggle, no command-palette command, no sidebar chip. (`glossary.rs` (rewrite), `commands.rs` (`cmd_glossary_entries` + new `cmd_save_glossary_entries`), `notes.rs`, `lib.rs`, `Settings.svelte` (new pane), `glossaryHighlight.ts`, `NoteEditor.svelte`, `AppShell.svelte`, `NoteList.svelte`, `CommandPalette.svelte`, `tauri.ts`, `types.ts`, `index.css`, en/es/sv)
- **Settings → About → Check for updates.** Manual button that asks GitHub Releases for the latest tag and shows whether you're up to date, with a link to the release notes when a newer version is out. No auto-check, no telemetry. (`updates.rs`, `commands.rs`, `Settings.svelte`, en/es/sv)
- **Personas v2 — Mode + Persona collapsed into one concept.** "Mode" and "Persona" are now a single picker with seven choices: **Basic**, **Default**, Writer, Researcher, Developer, Clinician, Cooking. Each persona is a manifest + lazy-loaded module under `src/plugins/<id>/`, so adding a new one means adding a manifest, not editing a registry. Legacy `yarrow.mode` localStorage migrates verbatim (with `path` → `default`).
- **Settings → Personas — search-driven palette.** Type-to-find with arrow-key navigation, "Recently used" pinned at top (max 4), Enter to pick, Esc to clear. Replaces the old two-section tile picker; scales as more personas land.
- **First-run wizard — "How do you want Yarrow to feel?"** Three tiles — Minimal / Full Yarrow / Tailored. Tailored reveals the persona grid; the other two pick implicitly.
- **Persona Actions popover.** Click the persona pill in the status bar → an Obsidian-style command list filtered to the active persona pops up. Search box, arrow-key navigation, Enter to pick, Esc to dismiss. Universal "Switch persona…" and "Open command palette…" rows always at the bottom. (`PersonaActionsPanel.svelte`, `AppShell.svelte`, en/es/sv)
- **Slash-command panel in the editor.** Type `/` at start of line or after whitespace → panel of formatting insertions (heading, callout, table, math, …), linking helpers (`[[`, `![[`, `#`, `??`), and app-level actions (palette, scratchpad, focus, history). Persona-specific commands surface at the top — Researcher gets *open questions* / *sources* / *new source*; Developer gets *decisions* / *new ADR* / *toggle code highlight*; etc. Modeled on Obsidian's `/` panel. (`Editor/extensions/slashCommands.ts`, `NoteEditor.svelte`, `AppShell.svelte`)

### Fixed

- **Paper-warmth slider no longer warps layout in Roomy/Compact.** Moved the warmth `filter` from `body` to `#root` — when both lived on `body`, `filter` made `#root`'s `position: fixed; width: 100%` resolve against body's counter-zoomed width instead of the viewport, leaving a strip of blank space (Roomy) or overshooting (Compact). (`index.css`, `paperPrefs.svelte.ts`)
